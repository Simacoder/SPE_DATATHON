...
class WellClassificationPipeline:
    ...
    def train_xgboost_model(self):
        """Train XGBoost model with hyperparameter tuning and robust evaluation"""
        print("\nTraining XGBoost model with hyperparameter tuning...")

        # Prepare target variables with zero-based encoding
        y_encoded = {}
        valid_targets = []

        for target_col in self.target_columns:
            le = LabelEncoder()
            encoded = le.fit_transform(self.target_data[target_col].astype(str))
            
            # Ensure labels start from 0 (XGBoost requirement)
            if encoded.min() > 0:
                encoded = encoded - encoded.min()
                
            y_encoded[target_col] = encoded
        ...
        train_indices, test_indices = train_test_split(
            ...
            test_size=0.2,
            random_state=42,
            stratify=y_encoded[valid_targets[0]]
        )
        ...
        for target_col in valid_targets:
            ...
            if len(present_classes) > 1:
                ...
                f1 = precision = recall = 1.0 if len(np.unique(y_train)) == 1 else 0.0
            ...
            if len(present_classes) > 1:
                ...
                print(f"\nSingle class in test set: {class_names[present_classes[0]]}")

        # Final training on full dataset
        print("\nTraining final models on full dataset...")
        for target_col in valid_targets:
            self.models[target_col].fit(X, y_encoded[target_col])
    ...
    def _generate_performance_report(self, metrics):
        ...
        report.append("-"*50)
        report.append(f"Mean Accuracy: {avg_accuracy:.3f}")
        report.append(f"Mean F1 Score: {avg_f1:.3f}")
...
if __name__ == "__main__":
    ...
    metrics = pipeline.train_xgboost_model()

    # Generate final report
    report = pipeline.generate_evaluation_report(metrics)
